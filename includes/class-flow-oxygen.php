...
/**
 * Oxygen 6 builder integration.
 *
 * Oxygen 6 is Breakdance-based (`BREAKDANCE_MODE === 'oxygen'`). It shares the
 * unofficial builder loader hook, but enters the builder via `?oxygen=builder&id=`
 * (Breakdance still uses `?breakdance=builder&id=`).
 */
class Oxygen {

	/** Same loader hook Breakdance fires — Oxygen 6 shares that template. */
	...
	public function render_panel(): void {
		...
		$template = (string) apply_filters(
			'flow_ew_oxygen_panel_template',
			FLOW_EW_PLUGIN_DIR . 'templates/oxygen/panel.php'
		);
	...
	private function enqueue_builder_assets( int $post_id ): void {
		...
		Assets::enqueue_builder_bundle( 'oxygen' );
	}
	...
	public function print_builder_assets(): void {
		...
		// Oxygen's loader template bypasses wp_head/wp_footer, so print Flow
	...
	private function is_builder_request(): bool {
		...
		$mode = sanitize_key( wp_unslash( $_GET['oxygen'] ) );
