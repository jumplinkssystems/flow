...
function applyReviewPayload(
	...
	if ( next && next.reviewer && next.reviewer.id ) {
		const found = reviewers.find(
			( u ) => u.id === Number( next.reviewer.id )
		);
		if ( found ) {
...
function FlowDataLoader() {
	...
	const loadReviewData = useCallback( () => {
		Promise.all( [
			...
			apiFetch( { url: `${ restUrl }/reviews/${ postId }` } ).catch(
				() => undefined
			),
		] )
