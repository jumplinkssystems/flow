...
import { EMAIL_SENTINEL, parseInviteValue } from '../../shared/invite-value';

const flowEW = getConfig();
const { restUrl, postId, currentUserCan, i18n, pendingReviewer } = flowEW;
...
export default function ReviewerField() {
	...
	const reviewers = useSelect(
		( sel ) => sel( STORE_NAME ).getReviewers(),
		[]
	);
	...
	const isUnsavedNewPost = useSelect(
		( sel ) =>
			'auto-draft' === sel( 'core/editor' )?.getCurrentPost?.()?.status,
		[]
	);
	const pendingReviewerData =
		! hasReviewer && isUnsavedNewPost && pendingReviewer
			? pendingReviewer
			: null;
	...
	const inviteFallback = __( 'Invite %s', 'jumplinks-editorial-workflow' );
		...
		<PanelRow>
			<div style={ { width: '100%' } }>
				<HStack justify="space-between" className="flow-ew-label-row">
					<span className="flow-ew-field-label">
						{ i18n.reviewer }
					</span>
				...
				{ hasReviewer && ! isEditing && (
					<div className="flow-ew-reviewer-card">
						...
						{ canCopyInvite && (
							<InviteLinkCopy reviewId={ reviewId } />
						) }
						{ canAssign && (
							<Button
								icon="no-alt"
				...
				{ pendingReviewerData && (
					<>
						<div className="flow-ew-reviewer-card flow-ew-reviewer-card--pending">
							<div className="flow-ew-reviewer-card__main">
								{ pendingReviewerData.avatar_url && (
									<img
								...
								<Text size="13" className="flow-ew-truncate">
									{ pendingReviewerData.name }
								</Text>
							</div>
						</div>
						<p className="flow-ew-reviewer-pending-hint">
							{ i18n.pendingReviewerHint }
						</p>
					</>
				) }
