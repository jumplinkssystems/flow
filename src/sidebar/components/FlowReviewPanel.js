...
export default function FlowReviewPanel() {
	...
	const isChangesRequested =
		...
		! review?.has_pending_reviewers;
	const reviewerId = review
		? Number( review.reviewer?.id || review.reviewer_id )
		: 0;
	...
	const approve = useCallback( async () => {
		...
		try {
			...
			createSuccessNotice(
				__( '✔ Approval recorded.', 'jumplinks-editorial-workflow' ),
				{ type: 'snackbar', isDismissible: true }
			);
		} catch ( err ) {
			createErrorNotice(
				err?.message ||
					__( 'Failed to approve.', 'jumplinks-editorial-workflow' ),
				{ isDismissible: true }
			);
		} finally {
			setLoading( false );
	...
	const requestChanges = useCallback( async () => {
		...
		try {
			...
			createSuccessNotice(
				__( '✔ Changes requested.', 'jumplinks-editorial-workflow' ),
				{ type: 'snackbar', isDismissible: true }
			);
		} catch ( err ) {
			createErrorNotice(
	...
	const revokeApproval = useCallback( async () => {
		...
		try {
			...
			createSuccessNotice(
				__( 'Approval revoked.', 'jumplinks-editorial-workflow' ),
				{ type: 'snackbar', isDismissible: true }
			);
		} catch ( err ) {
			createErrorNotice(
	...
	const resubmit = useCallback( async () => {
		...
		try {
			...
			createErrorNotice(
				err?.message ||
					__( 'Failed to resubmit.', 'jumplinks-editorial-workflow' ),
				{ isDismissible: true }
			);
		} finally {
			setLoading( false );
	...
	const reviewerFieldSlot = applyFilters(
		'flow_ew_reviewer_field_slot',
		null,
		{ review, loading, setReview, setLoading, restUrl }
	);
	// Free's "Unlock multiple reviewers" upsell (assets/js/flow-ew-upsell-reviewer.js) renders here.
	const reviewerFieldExtras = applyFilters(
					...
					{ isPendingSend && currentUserCan.assignReviewer && (
						<>
							...
							{ sendDisabled && sendDisabledHint && (
								<p className="flow-ew-actions__hint">
									{ sendDisabledHint }
								</p>
							) }
						</>
