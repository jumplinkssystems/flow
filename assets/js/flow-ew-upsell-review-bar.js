...
( function () {
	...
	const data = window.flowEwUpsellReviewBar;
	if ( ! data ) {
		return;
	}
	...
	const BAR_HEIGHT_VAR = '--flow-ew-upsell-bar-height';
	const BAR_HEIGHT_PX = '40px';
	...
	function mountBar() {
		...
		if ( document.getElementById( 'flow-ew-review-upsell-host' ) ) {
			return;
		}
		...
		const parts = String( data.message || '' ).split( '%s' );
		...
		if ( data.coupon ) {
			...
			text.appendChild(
				document.createTextNode(
					parts.length > 1 ? parts.slice( 1 ).join( '%s' ) : ''
				)
			);
